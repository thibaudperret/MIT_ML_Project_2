...
def extract_features(img):
    feat_m = np.mean(img, axis=(0,1)) #dim 3, mean R/G/B 
    feat_v = np.var(img, axis=(0,1)) #dim 3, var R/G/B
    feat = np.append(feat_m, feat_v) 
    return feat #dim (6,)

# Extract 2-dimensional features consisting of average gray color as well as variance
def extract_features_2d(img):
    feat_m = np.mean(img) #gray image pixel color coded with 1 variable so dim = 1 
    feat_v = np.var(img)
    feat = np.append(feat_m, feat_v)
    return feat
